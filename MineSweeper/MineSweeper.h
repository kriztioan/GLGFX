...
private:
  ...
  bool OnUserCreate() final {
    ...
    better_rand.seed( std::chrono::system_clock::now().time_since_epoch().count() );
    ...
    std::ifstream ifstr( L".minesweeper" );
    if( ifstr.good() ) {
      ifstr >> fBestTime;
      ifstr.close();
    } else fBestTime = 999.0f;
  ...
  bool OnUserUpdate( float fElapsedTime ) final {
    ...
    if( GameState == GameStateRunning ) {
      fTimer += fElapsedTime;
      if( fTimer > 999.0f ) fTimer = 999.0f;
    }
    ...
    if( GameState == GameStateRunning ) {
      ...
      if( MouseUp( GLUT_RIGHT_BUTTON, nMouseX, nMouseY ) &&
        ...
        if(nMouseX >= 0 &&
           ...
           nMouseY >= 0 &&
           nMouseY < nRows && cPlayer[ nMouseX + nMouseY * nCols ] != 1)
          cPlayer[ nMouseX + nMouseY * nCols ] = cPlayer[ nMouseX + nMouseY * nCols ] == 2 ? 0 : 2;
      }
    }
  ...
  void Logic() {
    ...
    if( GameState != GameStateRunning ) return;
    ...
    for( int x = 0; x < nCols; x++ ) {
      for( int y = 0; y < nRows; y++ ) {
        ...
        if( cPlayer[ x + y * nCols ] == 1 && nField[ x + y * nCols ] == 0 ) {
          ...
          if( y > 0 ) {
            if( x > 0 ) cPlayer[ ( x - 1 ) + ( y - 1 ) * nCols ] = 1;         // top-left
            cPlayer[ x + ( y - 1 ) * nCols ] = 1;                             // top
            if( x < nCols - 1 ) cPlayer[ ( x + 1 ) + ( y - 1 ) * nCols ] = 1; // top-right
          }
          if( x < nCols - 1 ) {
            cPlayer[ ( x + 1 ) + y * nCols ] = 1;                              // right
            if( y < nRows - 1 ) cPlayer[ ( x + 1 ) + ( y + 1 ) * nCols ] = 1;  // bottom-right
          }
          if( y < nRows - 1 ) {
            cPlayer[ x + ( y + 1 ) * nCols ] = 1;                              // bottom
            if( x > 0 ) cPlayer[ ( x - 1 ) + ( y + 1 ) * nCols ] = 1;          // bottom-left
          }
          if( x > 0 ) cPlayer[ ( x - 1 ) + y * nCols ] = 1;                    // left
        }
      }
    ...
    if( nCheck == 0 && nFlags == nBombs ) {
      ...
      if( fTimer < fBestTime ) {
        fBestTime = fTimer;
        std::ofstream ofstr( L".minesweeper" );
        if( ofstr.good() ) {
          ofstr << fBestTime;
          ofstr.close();
        }
  ...
  void Draw() {
    ...
    for( int x = 0; x < nCols; x++ ) {
      for( int y = 0; y < nRows; y++ ) {
        if( cPlayer[ x + y * nCols ] == 0 )
          DrawBitmap( *Tiles, x * nCellSize, y * nCellSize + nFieldOffsetY, 0, 0, nCellSize, nCellSize );
        else if( cPlayer[ x + y * nCols ] == 1 ) {
          DrawBitmap( *Tiles, x * nCellSize, y * nCellSize + nFieldOffsetY, nCellSize, 0, nCellSize, nCellSize );
          if( nField[ x + y * nCols ] > 0 )
            DrawString( x * nCellSize + 4, y * nCellSize + 4 + nFieldOffsetY, std::to_string( nField[ x + y * nCols ] ),
                        nColors[ nField[ x + y * nCols ] ] );
        }
        if( GameState > GameStateRunning && bField[ x + y * nCols ] )
          DrawBitmap( *Tiles, x * nCellSize, y * nCellSize + nFieldOffsetY, 2 * nCellSize, 0, nCellSize, nCellSize );
        if( cPlayer[ x + y * nCols ] == 2 )
          DrawBitmap( *Tiles, x * nCellSize, y * nCellSize + nFieldOffsetY, 3 * nCellSize, 0, nCellSize, nCellSize );
      }
    }
    std::stringstream ss;
    ...
    if( GameState == GameStateOver )
      DrawString( ( ScreenWidth() - 72 ) / 2, ( nFieldOffsetY - 8 ) / 2, "GAME OVER", FG_RED );
    if( GameState == GameStateWinner )
      DrawString( ( ScreenWidth() - 48 ) / 2, ( nFieldOffsetY - 8 ) / 2, "WINNER", FG_RED );
  ...
  void Reset() {
    ...
    for( int n = 0; n < nCols * nRows / 10; n++ ) {
      ...
      while( bField[ i ] ) i = better_rand() % ( nCols * nRows );
      bField[ i ] = true;
    }
    ...
    for( int x = 0; x < nCols; x++ ) {
      for( int y = 0; y < nRows; y++ ) {
        if( bField[ x + y * nCols ] ) continue;
        if( y > 0 ) {
          if( x > 0 && bField[ ( x - 1 ) + ( y - 1 ) * nCols ] ) ++nField[ x + y * nCols ];          // top-left
          if( bField[ x + ( y - 1 ) * nCols ] ) ++nField[ x + y * nCols ];                           // top
          if( x < nCols - 1 && bField[ ( x + 1 ) + ( y - 1 ) * nCols ] ) ++nField[ x + y * nCols ];  // top-right
        }
        if( x < nCols - 1 ) {
          if( bField[ ( x + 1 ) + y * nCols ] ) ++nField[ x + y * nCols ];                           // right
          if( y < nRows - 1 && bField[ ( x + 1 ) + ( y + 1 ) * nCols ] ) ++nField[ x + y * nCols ];  // bottom-right
        }
        if( y < nRows - 1 ) {
          if( bField[ x + ( y + 1 ) * nCols ] ) ++nField[ x + y * nCols ];                           // bottom
          if( x > 0 && bField[ ( x - 1 ) + ( y + 1 ) * nCols ] ) ++nField[ x + y * nCols ];          // bottom-left
        }
        if( x > 0 && bField[ ( x - 1 ) + y * nCols ] ) ++nField[ x + y * nCols ];                    // left
      }
    }
  }
  ...
  enum GameStates {
    GameStateRunning, GameStateOver, GameStateWinner
  };
  ...
  static constexpr short nColors[8] = { 0, FG_BLUE, FG_GREEN, FG_RED, FG_NAVY, FG_MAROON, FG_BROWN, FG_YELLOW };
